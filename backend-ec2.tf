module "backend_public_ec2_instance" {
  ...
  vpc_security_group_ids = [aws_security_group.backend_public_ssh_ping_traceroute.id]
...
module "backend_private_ec2_instance" {
  ...
  vpc_security_group_ids = [aws_security_group.backend_private_ssh_ping_traceroute.id]
